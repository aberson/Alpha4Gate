...
*Generated by /build-phase on 2026-08-10. Append-only; re-running the phase adds new items below, never modifies existing ones.*

### M1: Real-SC2 smoke gate
- **Source step:** Step EV.4 (from this plan's §7)
- **Issue:** #294
- **Commands to run:**
  ...
  **Pre-flight — serialize against the other pending soaks (plan §0 P-4, P-2).** Only one evolve
  run may exist machine-wide; EJ.7 (#288), EJ.8 (#289), EL.7 (#279) and Phase 7 Step 6 (#280) use
  ...
  **Negative case 1 — concurrency rejection (D-2).** Exits in about a second.

  ```powershell
  uv run python scripts/evolve.py --viewer --concurrency 2
  ```

  **Negative case 2 — missing `[viewer]` extra degrades, never crashes (D-3).** Note that
  `uv run` without `--extra viewer` re-syncs the environment without pygame; the next
  `--extra viewer` invocation restores it. Let it start, confirm the WARNING, then stop it.

  ```powershell
  uv run python scripts/evolve.py --pool-size 2 --games-per-eval 3 --hours 0.1 --no-commit --viewer
  ```

  **Items (a)-(d) — the flagged smoke run.** About 30 minutes. Watch the container.

  ```powershell
  uv run --extra viewer python scripts/evolve.py --pool-size 2 --games-per-eval 3 --hours 0.5 --no-commit --viewer
  ```

  Partway through, close the themed container and leave the run going. Then confirm rows are
  still being appended after the window closed — this is item (d), the only direct proof that
  detach-and-continue-headless works:

  ```powershell
  (Get-Content data/evolve_results.jsonl | Measure-Object -Line).Lines
  # wait a few minutes, then re-run the line above and confirm the count grew
  ```

  **Defaults-off control — 10 minutes, confirms zero behavior change.**

  ```powershell
  uv run python scripts/evolve.py --pool-size 2 --games-per-eval 3 --hours 0.5 --no-commit
  ```

  **Item (e) — the acceptance gate.** Click the dev-observatory `run-evolution` button if the
  control plane is up; otherwise run the identical command the registry holds, bare, with no
  arguments so it uses its own `-Hours 4` default:

  ```powershell
  .\scripts\launch-evolve.ps1
  ```

  With the run up, verify the guard by invoking it a second time (e3):

  ```powershell
  .\scripts\launch-evolve.ps1
  ```

  **This run is real.** The launcher passes no `--no-commit`, so it can promote a version, flip
  `bots/current`, and auto-commit `[evo-auto]` to `master-plan/phase-ev`. That is expected (D16) —
  do not "fix" it by adding `--no-commit`, because (e) exists to exercise the button exactly as
  the registry invokes it. Stop the run once (e1)-(e4) are observed by **closing the evolve
  CONSOLE window**. Closing the container only detaches; the dashboard Stop button is not wired;
  and **never press Ctrl+C on a `--viewer` run** — the loop runs off the main thread, so
  burnysc2's SIGINT kill-switch is never armed and Ctrl+C can orphan SC2 processes.

- **What you're looking for:**

  | Check | Expected outcome |
  |---|---|
  | Pre-flight: branch | `master-plan/phase-ev`; no other evolve python/SC2 processes running |
  | Negative 1: `--viewer --concurrency 2` | Exits immediately, non-zero, with the guidance message naming the mutual exclusion. No window, no games |
  | Negative 2: no `[viewer]` extra | Run STARTS headless after a WARNING naming the missing dependency. A crash or traceback is a failure |
  | (a) Container opens | Themed container with a background image and a stats bar — not a raw SC2 window |
  | (b) Fitness phase renders | Both SC2 clients reparent into the two panes; overlay labels read `cand_<hash>` vs the parent version |
  | (c) Idle phases | During Claude pool-generation and stack-apply the container shows placeholders and stays responsive — no freeze, no white window |
  | (d) Detach | After closing the container the evolve process stays alive and `data/evolve_results.jsonl` gains rows. The `[selfplay_viewer]` "orphaned SC2 processes may remain" warning is expected and wrong here; the `[evolve]` stderr line immediately after it is the correction |
  | Defaults-off control | Raw SC2 windows as before EV; `evolve_results.jsonl` row shapes identical to the viewer run |
  | (e1) **HARD GATE** — real match in the container | An actual SC2 game visibly rendered inside the themed container, launched from the button/launcher. Not "the window opened". Wait for it: pool generation runs a Claude prompt first, so first paint can be several minutes out |
  | (e2) Dashboard | Opens on the Evolution tab and shows the run |
  | (e3) Already-running guard | The second `launch-evolve.ps1` invocation refuses instead of starting a competing run |
  | (e4) Close container | Both the run and the dashboard stay alive |

  **Verdict rule:** (e1) is the phase's acceptance condition. If it does not happen, Phase EV has
  not met its definition of done regardless of how many unit tests pass, and EV.5 is blocked.
  Record the per-item verdicts as smoke evidence under `documentation/soak-test-runs/`.
