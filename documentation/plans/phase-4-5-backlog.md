...
| (post-triage) | minor | Several Alpha4Gate-specific lessons from soak run #1 have no wiki home. A fresh-context Claude or new contributor would re-discover them. Cross-project patterns (silent exception fallthrough, duplicate shape constants, tee discipline, Python stdout buffering) are already captured in `~/dev/docs/lessons-learned.md` and explicitly do NOT belong in the Alpha4Gate wiki — that doc is the source of truth for cross-project knowledge. The project-specific lessons that DO belong in the wiki: (a) SQLite thread-safety contract — daemon writes from a background thread, so connections must use `check_same_thread=False`; (b) eval game persistence semantics — eval games can drop silently if the game record doesn't reach the DB, check eval game count against expected; (c) promotion gate first-cycle behavior — `manifest.best` must be pre-seeded or the gating comparison code path never runs (Finding #11); (d) alerts pipeline T0 synthetic test requirement — pipeline can be silently broken without it (Finding #14); (e) daemon state machine semantics — once #8 splits `state` into `training_cycle`/`evaluating`/`promoting`/`idle`, the enum is part of the public observability contract and the dashboard depends on the exact values. | Attach gotcha sub-sections to existing wiki pages, NOT a new `gotchas.md`. Mapping: SQLite contract → `documentation/wiki/architecture.md`; eval game persistence → `documentation/wiki/evaluation-pipeline.md`; promotion gate first-cycle → `documentation/wiki/promotions.md`; alerts T0 → `documentation/wiki/monitoring.md`; daemon state machine → `documentation/wiki/training-pipeline.md`. **Schedule: after Phase 4.5 Step 5 completes** — gotchas (a), (b), and (e) can only be written accurately once #66, #67, and #8 land respectively, and writing them now would create stale text that contradicts the fixed code. Gotchas (c) and (d) can be written at any time but should batch with the others. `/repo-update`'s wiki coverage check picks up the new sub-sections on the next phase wrap-up, so no separate scheduling beat is needed. Style: follow `feedback_wiki_style.md` — TL;DR blockquote at section top, reference tone for Claude, link to the relevant source file with line numbers. |
