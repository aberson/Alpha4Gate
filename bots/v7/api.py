...
from bots.v7.error_log import get_error_log_buffer, install_error_log_handler
from bots.v7.learning.daemon import DaemonConfig, TrainingDaemon
from bots.v7.web_socket import (
    ConnectionManager,
...
def configure(
    ...
    *,
    ...
    from bots.v7.config import Settings
...
async def _lifespan(application: FastAPI) -> AsyncIterator[None]:
    ...
    if _daemon is None:
        from bots.v7.config import load_settings
        from bots.v7.learning.daemon import load_daemon_config
...
async def create_build_order(order: dict[str, Any]) -> dict[str, Any]:
    ...
    from bots.v7.build_orders import (
        BuildOrder,
        load_build_orders,
        save_build_orders,
        slug_from_name,
...
async def delete_build_order(order_id: str) -> dict[str, Any]:
    ...
    from bots.v7.build_orders import load_build_orders, save_build_orders
...
async def get_decision_log() -> dict[str, Any]:
    ...
    """
    from bots.v7.audit_log import _apply_ui_aliases
...
async def get_training_status() -> dict[str, Any]:
    ...
    from bots.v7.learning.checkpoints import get_best_name, list_checkpoints
...
async def debug_raise_error(
    ...
    debug_logger = logging.getLogger("bots.v7.debug")
...
async def get_training_reward_trends(
    ...
    """
    from bots.v7.learning.reward_aggregator import aggregate_reward_trends
...
def _get_evaluator() -> Any:
    ...
    if _evaluator is None:
        from bots.v7.config import Settings
        from bots.v7.learning.database import TrainingDB
        from bots.v7.learning.evaluator import ModelEvaluator
...
def _get_promotion_manager() -> Any:
    ...
    if _promotion_manager is None:
        from bots.v7.learning.promotion import PromotionConfig, PromotionManager
...
def _get_promotion_logger() -> Any:
    ...
    if _promotion_logger is None:
        from bots.v7.learning.promotion import PromotionLogger
...
async def manual_rollback(request: dict[str, Any]) -> JSONResponse | dict[str, Any]:
    ...
    """
    from bots.v7.learning.checkpoints import get_best_name
    from bots.v7.learning.database import TrainingDB
    from bots.v7.learning.rollback import RollbackConfig, RollbackDecision, RollbackMonitor
...
async def get_system_substrate() -> dict[str, Any]:
    ...
    """
    from bots.v7.system_info import get_substrate_info
...
async def get_system_wsl_processes() -> dict[str, Any]:
    ...
    """
    from bots.v7.system_info import get_wsl_processes
...
async def get_system_resources() -> dict[str, Any]:
    ...
    """
    from bots.v7.system_info import get_resources
...
async def get_processes() -> dict[str, Any]:
    ...
    from bots.v7.process_registry import full_status
...
async def restart_server() -> dict[str, str]:
    ...
    cmd = [sys.executable, "-m", "bots.v7.runner", "--serve"]
    if _daemon is not None and _daemon.is_running():
        _daemon.stop()
