...
from bots.v7.build_orders import BuildOrder, BuildSequencer, default_4gate
...
class DecisionEngine:
    ...
    def _compute_next_state(self, snapshot: GameSnapshot) -> StrategicState:
        ...
        # DEFEND/FORTIFY stuck-state timeout: if we've been in DEFEND or
        # FORTIFY for too long with no visible enemy army, break out to EXPAND.
        if self._state in (StrategicState.DEFEND, StrategicState.FORTIFY):
            if self._defend_entered_at is not None and (
                snapshot.game_time_seconds - self._defend_entered_at
                >= self.DEFEND_STUCK_TIMEOUT_SECONDS
                and snapshot.enemy_army_supply_visible == 0
            ):
                self._defend_entered_at = None
