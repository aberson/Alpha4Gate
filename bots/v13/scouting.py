...
class ScoutManager:
    ...
    def escort_army_observer(
        self, bot: BotAI, army_center: tuple[float, float] | None
    ) -> None:
        if army_center is None or not bot.units(UnitTypeId.OBSERVER):
            return
        observer = bot.units(UnitTypeId.OBSERVER).first
        distance = (
            (observer.position.x - army_center[0]) ** 2
            + (observer.position.y - army_center[1]) ** 2
        ) ** 0.5
        if distance > 8.0:
            observer.move(Point2(army_center))
