...
from bots.v13.build_orders import BuildOrder, BuildSequencer, default_4gate
...
class DecisionEngine:
    ...
    def __init__(
        ...
        self._supply_history: collections.deque[tuple[float, float]] = collections.deque(
            maxlen=12
        )
        self._panic_defend_until: float = 0.0
    ...
    DEFEND_INTERRUPT_THRESHOLD: int = 15
    ...
    def _compute_next_state(self, snapshot: GameSnapshot) -> StrategicState:
        ...
        self._supply_history.append(
            (snapshot.game_time_seconds, float(snapshot.army_supply))
        )
        if snapshot.game_time_seconds < self._panic_defend_until:
        ...
        # Fast DEFEND recovery: army rebuilt and no enemy pressure near bases
        if (
            self._state == StrategicState.DEFEND
            and snapshot.army_supply >= self.ATTACK_ARMY_SUPPLY
            and not snapshot.enemy_army_near_base
        ):
            _logger.info("defend-recovery: army rebuilt with no enemy near base")
            self._defend_entered_at = None
            self._recently_retreated = False
            return StrategicState.ATTACK

        # Half-timeout DEFEND exit: stabilised with supply advantage
        if (
            self._state == StrategicState.DEFEND
            and self._defend_entered_at is not None
            and snapshot.game_time_seconds - self._defend_entered_at >= 45.0
            and snapshot.army_supply > snapshot.enemy_army_supply_visible * 1.2
        ):
            _logger.info("defend-recovery: supply advantage after 45s")
            self._defend_entered_at = None
            self._recently_retreated = False
            return StrategicState.ATTACK
        ...
        if (
            self._state == StrategicState.ATTACK
            and len(self._supply_history) >= 6
        ):
            current_supply = float(snapshot.army_supply)
            cutoff_time = snapshot.game_time_seconds - 20.0
            oldest_supply_in_window: float | None = None
            for hist_time, hist_supply in self._supply_history:
                if hist_time <= cutoff_time:
                    oldest_supply_in_window = hist_supply
                else:
                    break
            if oldest_supply_in_window is None:
                oldest_supply_in_window = self._supply_history[0][1]
            if (
                oldest_supply_in_window > 0
                and (current_supply / oldest_supply_in_window) < 0.75
            ):
                self._panic_defend_until = snapshot.game_time_seconds + 60.0
                return StrategicState.DEFEND
