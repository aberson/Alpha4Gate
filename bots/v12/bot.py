...
from bots.v12.army_coherence import ArmyCoherenceManager
from bots.v12.build_backlog import BuildBacklog
from bots.v12.build_orders import BuildOrder
from bots.v12.claude_advisor import ClaudeAdvisor, build_prompt
...
from bots.v12.commands.dispatch_guard import DispatchGuard
from bots.v12.console import print_status
from bots.v12.decision_engine import DecisionEngine, GameSnapshot, StrategicState
from bots.v12.fortification import FortificationManager
from bots.v12.give_up import (
    GIVE_UP_PROB_THRESHOLD,
    GIVE_UP_WINDOW,
    should_give_up,
)
from bots.v12.learning import winprob_heuristic
from bots.v12.learning.features import _FEATURE_SPEC
from bots.v12.learning.neural_engine import DecisionMode, NeuralDecisionEngine
from bots.v12.learning.rewards import RewardCalculator
from bots.v12.learning.threat_classes import THREAT_CLASS_MAP
from bots.v12.logger import GameLogger
from bots.v12.macro_manager import MacroDecision, MacroManager
from bots.v12.micro import MicroController
from bots.v12.observer import observe
from bots.v12.scouting import ScoutManager
from bots.v12.web_socket import queue_broadcast, queue_command_event
...
_SENTRY_WARP: tuple[UnitTypeId, AbilityId] = (
    UnitTypeId.SENTRY,
...
class Alpha4GateBot(BotAI):
    ...
    async def on_step(self, iteration: int) -> None:
        ...
        # --- Auto-place a Shield Battery at each new expansion ---
        await self._fortification_manager.auto_battery(self)
        ...
        army_units = self.units.exclude_type({UnitTypeId.PROBE, UnitTypeId.OBSERVER})
        army_center: tuple[float, float] | None = (
            (float(army_units.center.x), float(army_units.center.y))
            if army_units
            else None
        )
        self.scout_manager.escort_army_observer(self, army_center)
    ...
    def _evaluate_fortification(self, snapshot: GameSnapshot) -> list[MacroDecision]:
        ...
        decisions: list[MacroDecision] = self._fortification_manager.evaluate(
        ...
        if self.decision_engine.state == StrategicState.DEFEND:
            threatened_nexus_tags = [
                int(n.tag)
                for n in self.townhalls
                if any(
                    e.position.distance_to(n.position) < 15
                    for e in self.enemy_units
                )
            ]
            decisions.extend(
                self._fortification_manager.reactive_defend_emplacements(
                    threatened_nexus_tags=threatened_nexus_tags,
                    existing_batteries=existing_batteries,
                    existing_cannons=existing_cannons,
                    has_forge=has_forge,
                    has_cybernetics=has_cyber,
                )
            )
    ...
    async def _produce_army(self) -> None:
        ...
        gas_flooding: bool = int(self.vespene) > 600
        ...
        has_templar_archives: bool = self.structures(
            UnitTypeId.TEMPLARARCHIVE,
        ).ready.exists
        has_robo_bay: bool = bool(self.structures(UnitTypeId.ROBOTICSBAY).ready)
        ...
        if gas_flooding and has_templar_archives:
            want_high_templar = True
        want_sentry: bool = gas_flooding and not has_templar_archives
        if want_high_templar:
            warp_priority: list[tuple[UnitTypeId, AbilityId]] = [
                _HIGH_TEMPLAR_WARP, *_WARPGATE_ABILITIES,
            ]
        elif want_sentry:
            warp_priority = [_SENTRY_WARP, *_WARPGATE_ABILITIES]
        else:
            warp_priority = list(_WARPGATE_ABILITIES)
        for wg in self.structures(UnitTypeId.WARPGATE).ready:
            ...
            for unit_id, ability in warp_priority:
                if (
                    gas_flooding
                    and has_templar_archives
                    and unit_id == UnitTypeId.ZEALOT
                ):
                    unit_id = UnitTypeId.HIGHTEMPLAR
                    ability = AbilityId.WARPGATETRAIN_HIGHTEMPLAR
                if (
                    gas_flooding
                    and has_robo_bay
                    and unit_id == UnitTypeId.STALKER
                ):
                    continue
    ...
    async def _run_micro(self, state: StrategicState) -> None:
        ...
        army_center_pt: Point2 | None = (
            Point2.center([u.position for u in army]) if army else None
        )
        self.micro_controller._blink_retreat_low_shield_stalkers(self, army_center_pt)
    ...
    def _resolve_attack_rally(
        ...
        support_units: dict[str, int] = {
            "colossus": int(self.units(UnitTypeId.COLOSSUS).amount),
            "disruptor": int(self.units(UnitTypeId.DISRUPTOR).amount),
            "archon": int(self.units(UnitTypeId.ARCHON).amount),
            "high_templar_ready": sum(
                1
                for ht in self.units(UnitTypeId.HIGHTEMPLAR)
                if ht.energy >= 75
            ),
        }
        if cm.should_attack(own_supply, enemy_supply, support_units=support_units):
            cm.update_staging_timer(self.time, is_staging=False)
