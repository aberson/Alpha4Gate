...
class ArmyCoherenceManager:
    ...
    def should_attack(
        ...
        support_units: dict[str, int] | None = None,
    ) -> bool:
        ...
        if self._recently_retreated:
            ...
            required_ratio = min(required_ratio, self.MAX_REQUIRED_RATIO)

        if enemy_visible_supply > 0 and own_supply >= enemy_visible_supply * required_ratio:
            # Splash-readiness gate: at contested supply require at least one splash/HT unit.
            if (
                own_supply < enemy_visible_supply * 1.5
                and support_units is not None
                and sum(support_units.values()) == 0
            ):
                return False
            self._recently_retreated = False
            return True
