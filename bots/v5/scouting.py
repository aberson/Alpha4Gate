...
class ScoutManager:
    ...
    def __init__(self) -> None:
        ...
        self._observer_last_target: dict[int, Point2] = {}
    ...
    def escort_army_observer(self, bot: BotAI, army_center: Point2 | None) -> None:
        """Keep an idle Observer escorting the army center for vision discipline."""
        if army_center is None:
            return
        observers = bot.units(UnitTypeId.OBSERVER)
        if not observers:
            return
        obs = observers.closest_to(army_center)
        if obs.distance_to(army_center) <= 8:
            return
        if self._observer_last_target.get(obs.tag) == army_center:
            return
        obs.move(army_center)
        self._observer_last_target[obs.tag] = army_center
