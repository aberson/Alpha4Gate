...
class MacroManager:
    ...
    def _apply_chrono_boost(self, bot: BotAI) -> None:
        ...
        fallback_research_buildings: tuple[UnitTypeId, ...] = (
        ...
        try:
            wg_progress: float = float(
                bot.already_pending_upgrade(UpgradeId.WARPGATERESEARCH)
            )
        except (TypeError, AttributeError):
            wg_progress = 0.0
        used_target_tags: set[int] = set()
        for nexus in nexuses:
            if nexus.energy < 180:
                continue
            ...
            if 0 < wg_progress < 1:
                for core in bot.structures(UnitTypeId.CYBERNETICSCORE).ready:
                    if (
                        core.is_active
                        and not core.has_buff(BuffId.CHRONOBOOSTENERGYCOST)
                        and int(core.tag) not in used_target_tags
                    ):
                        target = core
                        break
            if target is None:
                for type_id in fallback_research_buildings:
                    for structure in bot.structures(type_id).ready:
                        if (
                            structure.is_active
                            and not structure.has_buff(BuffId.CHRONOBOOSTENERGYCOST)
                            and int(structure.tag) not in used_target_tags
                        ):
                            target = structure
                            break
                    if target is not None:
                        break
            if target is None:
                best_nexus: Unit | None = None
                best_sat: int = -1
                for n in nexuses:
                    if (
                        not n.is_active
                        or n.has_buff(BuffId.CHRONOBOOSTENERGYCOST)
                        or int(n.tag) in used_target_tags
                    ):
                        continue
                    sat = int(n.assigned_harvesters)
                    if sat > best_sat:
                        best_sat = sat
                        best_nexus = n
                target = best_nexus
            ...
            used_target_tags.add(int(target.tag))
            _log.info(
                "Chrono fallback: nexus energy=%d cast onto %s",
                int(nexus.energy),
                target.type_id.name,
            )
