...
class DecisionEngine:
    ...
    LOW_WINPROB_FORCE_DEFEND_STEPS: int = 20
    LOW_WINPROB_THRESHOLD: float = 0.15
    ...
    def __init__(
        ...
        self._low_winprob_streak: int = 0
    ...
    def _compute_next_state(self, snapshot: GameSnapshot) -> StrategicState:
        ...
        bot_obj: Any = getattr(self, "_bot", None)
        wp_raw: Any = getattr(bot_obj, "winprob_estimate", None)
        winprob: float | None = (
            float(wp_raw) if isinstance(wp_raw, (int, float)) else None
        )
        if winprob is not None and winprob < self.LOW_WINPROB_THRESHOLD:
            self._low_winprob_streak += 1
        else:
            self._low_winprob_streak = 0

        proposed = self._compute_proposed_state(snapshot)

        if (
            self._low_winprob_streak >= self.LOW_WINPROB_FORCE_DEFEND_STEPS
            and proposed == StrategicState.ATTACK
        ):
            _logger.info("forced defend: low winprob streak")
            return StrategicState.DEFEND

        return proposed

    def _compute_proposed_state(self, snapshot: GameSnapshot) -> StrategicState:
        """Determine the next state based on current game conditions."""
