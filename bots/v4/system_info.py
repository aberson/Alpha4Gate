...
def _build_substrate_info() -> dict[str, Any]:
    ...
    if _wsl_available():
        ...
        # NB: ``echo $SC2PATH`` through ``wsl -- bash -lc`` returns empty
        # even when the env var IS set (validated 2026-04-28 — ``env``
        # output shows the value, but bash parameter expansion silently
        # loses it through the wsl bridge).  Single-quoted awk scripts
        # also break (the wsl bridge strips the single quotes, so awk's
        # ``$1`` / ``$2`` get pre-expanded by bash to empty).
        # ``grep | cut`` survives because it needs no shell quoting.
        sc2_path = _run_wsl(
        ...
        if sc2_path and sc2_path.strip():
            ...
            # Probe via ``find`` so we don't hard-code the SC2 build
            # version (Base75689 = 4.10.0 today, may change).
            binary = _run_wsl(
                [
                    "bash", "-lc",
