...
from bots.v4.army_coherence import ArmyCoherenceManager
from bots.v4.build_backlog import BuildBacklog
from bots.v4.build_orders import BuildOrder
from bots.v4.claude_advisor import ClaudeAdvisor, build_prompt
from bots.v4.commands import (
    CommandExecutor,
    CommandMode,
    CommandSource,
    filter_executable,
...
from bots.v4.commands.dispatch_guard import DispatchGuard
from bots.v4.console import print_status
from bots.v4.decision_engine import DecisionEngine, GameSnapshot, StrategicState
from bots.v4.fortification import FortificationManager
from bots.v4.give_up import (
    GIVE_UP_PROB_THRESHOLD,
    GIVE_UP_WINDOW,
    should_give_up,
)
from bots.v4.learning import winprob_heuristic
from bots.v4.learning.features import _FEATURE_SPEC
from bots.v4.learning.neural_engine import DecisionMode, NeuralDecisionEngine
from bots.v4.learning.rewards import RewardCalculator
from bots.v4.learning.threat_classes import THREAT_CLASS_MAP
from bots.v4.logger import GameLogger
from bots.v4.macro_manager import MacroDecision, MacroManager
from bots.v4.micro import MicroController
from bots.v4.observer import observe
from bots.v4.scouting import ScoutManager
from bots.v4.web_socket import queue_broadcast, queue_command_event

if TYPE_CHECKING:
    from bots.v4.learning.database import TrainingDB
...
class Alpha4GateBot(BotAI):
    ...
    async def _run_scouting(self) -> None:
        ...
        # Escort army with an Observer for detection
        ...
        if army:
            ...
            army_center: Point2 | None = Point2((cx, cy))
        else:
            army_center = None
    ...
    def _resolve_attack_rally(
        ...
        support_units: dict[str, int] = {
            "colossus": len(self.units(UnitTypeId.COLOSSUS)),
            "archon": len(self.units(UnitTypeId.ARCHON)),
            "disruptor": len(self.units(UnitTypeId.DISRUPTOR)),
            "high_templar_ready": sum(
                1 for ht in self.units(UnitTypeId.HIGHTEMPLAR) if ht.energy >= 75
            ),
        }
        if cm.should_attack(
            own_supply, enemy_supply, support_units=support_units, game_time=self.time
        ):
            cm.update_staging_timer(self.time, is_staging=False)
