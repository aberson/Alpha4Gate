...
class MacroManager:
    ...
    def _check_supply(self, bot: BotAI) -> None:
        ...
        Computes pending supply demand from units in production and
        scales the buffer so multi-gateway warp-in cycles don't get
        supply-blocked.
        """
        ...
        pending_supply = 0
        for unit_type, cost in supply_costs:
            pending_supply += int(bot.already_pending(unit_type)) * cost

        effective_buffer = max(SUPPLY_BUFFER, pending_supply + 4)
        # Also keep the gateway-cycle buffer
        desired_buffer = max(effective_buffer, gateway_count * 2)
        ...
        pylon_cap = 3 if int(bot.supply_used) > 100 else 2
        for _ in range(min(pylons_needed, pylon_cap)):
    ...
    def _check_chrono_boost(self, bot: BotAI) -> None:
        """Chrono Boost Nexuses that are near energy cap to avoid waste.

        Priority: Nexus training probe > CyberneticsCore researching WarpGate >
        Forge upgrading > RoboticsFacility producing. Fallback: chrono the
        Nexus's own probe production.
        """
        ...
        for nexus in bot.townhalls.ready:
            ...
            # First: check if this Nexus itself is training a probe
            if not nexus.is_idle:
                try:
                    has_chrono = bool(nexus.has_buff(chrono_buff))
                except (TypeError, AttributeError):
                    has_chrono = False
                if not has_chrono:
                    self._pending.append(
                        MacroDecision(
                            action="chrono",
                            target=str(nexus.tag),
                            reason="Chrono Nexus probe production (energy near cap)",
                        )
                    )
                    continue

            # Then: find a busy priority structure without chrono
            chrono_issued = False
            for struct_type in priority_targets:
                for struct in bot.structures(struct_type).ready:
                    if struct.is_idle:
                        continue
                    try:
                        has_chrono = bool(struct.has_buff(chrono_buff))
                    except (TypeError, AttributeError):
                        has_chrono = False
                    if not has_chrono:
                        self._pending.append(
                            MacroDecision(
                                action="chrono",
                                target=str(struct.tag),
                                reason=(
                                    f"Chrono {struct_type.name}"
                                    f" (Nexus energy {int(nexus.energy)})"
                                ),
                            )
                        )
                        chrono_issued = True
            ...
            # Fallback: chrono the Nexus's own probe production even if idle
            if not chrono_issued and not nexus.is_idle:
                self._pending.append(
                    MacroDecision(
                        action="chrono",
                        target=str(nexus.tag),
                        reason="Chrono Nexus probe (fallback, energy near cap)",
                    )
                )
