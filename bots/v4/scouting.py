...
class ScoutManager:
    ...
    def escort_army_observer(self, bot: BotAI, army_center: Point2 | None) -> None:
        """Move the nearest Observer to escort the main army group."""
        ...
        army = [u for u in bot.units if not u.is_structure and u.type_id != UnitTypeId.PROBE]
        army_supply = len(army) * 2
        if army_supply < 10:
            return

        closest = observers.closest_to(army_center)
        if closest.distance_to(army_center) > 12.0:
            closest.move(army_center)
