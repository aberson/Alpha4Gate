...
from bots.v2.build_orders import BuildOrder, BuildSequencer, default_4gate
...
class DecisionEngine:
    ...
    def __init__(
        ...
        self._step_stuck_since: float = 0.0
        self._last_sequencer_index: int = 0
    ...
    def _compute_next_state(self, snapshot: GameSnapshot) -> StrategicState:
        ...
        if self._state == StrategicState.OPENING:
            if self._sequencer.is_complete:
                return StrategicState.EXPAND
            # Escape hatch: if stuck in OPENING for over 5 minutes, force EXPAND
            if snapshot.game_time_seconds > 300.0:
                return StrategicState.EXPAND
            # Step-skip: if a single build step hasn't advanced for 30s, skip it
            current_idx = self._sequencer.current_index
            if current_idx != self._last_sequencer_index:
                self._last_sequencer_index = current_idx
                self._step_stuck_since = snapshot.game_time_seconds
            elif (
                snapshot.game_time_seconds - self._step_stuck_since > 30.0
                and self._step_stuck_since > 0.0
            ):
                self._sequencer.advance()
                self._last_sequencer_index = self._sequencer.current_index
                self._step_stuck_since = snapshot.game_time_seconds
                if self._sequencer.is_complete:
                    return StrategicState.EXPAND
