...
class ScoutManager:
    ...
    def check_scout_alive(self, bot: BotAI) -> None:
        """Clear scout assignment if the scout unit is dead.

        Args:
            bot: BotAI instance with current game state.
        """
        if self._scout_tag is not None:
            if not any(u.tag == self._scout_tag for u in bot.units):
