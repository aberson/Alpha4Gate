...
class ScoutManager:
    ...
    def __init__(self) -> None:
        ...
        self._observer_patrol_targets: dict[int, Any] = {}
    ...
    def assign_observer_scouts(self, bot: BotAI) -> None:
        """Send idle Observers to patrol enemy expansion locations.

        Iterates idle Observers and assigns each to the nearest unvisited
        enemy expansion location. Tracks assignments to avoid duplicates.

        Args:
            bot: BotAI instance with current game state.
        """
        dead_tags = [
            tag for tag in self._observer_patrol_targets
            if tag not in {u.tag for u in bot.units(UnitTypeId.OBSERVER)}
        ]
        for tag in dead_tags:
            del self._observer_patrol_targets[tag]

        assigned_positions: set[int] = set()
        for target in self._observer_patrol_targets.values():
            loc: Any
            for i, loc in enumerate(bot.expansion_locations_list):
                if loc.distance_to(target) < 1.0:
                    assigned_positions.add(i)
                    break

        idle_observers = bot.units(UnitTypeId.OBSERVER).idle
        for obs in idle_observers:
            best_idx: int | None = None
            best_dist: float = float("inf")
            for i, loc in enumerate(bot.expansion_locations_list):
                if i in assigned_positions:
                    continue
                dist: float = obs.distance_to(loc)
                if dist < best_dist:
                    best_dist = dist
                    best_idx = i
            if best_idx is not None:
                target_loc: Any = bot.expansion_locations_list[best_idx]
                obs.patrol(target_loc)
                self._observer_patrol_targets[obs.tag] = target_loc
